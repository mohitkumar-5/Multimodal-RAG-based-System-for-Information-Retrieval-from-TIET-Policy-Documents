...
document.addEventListener("DOMContentLoaded", () => {
    // Restore Saved Theme
    const savedTheme = localStorage.getItem("tiet_theme");
    if (savedTheme === "light") {
...
function setupEventListeners() {
    ...
    if (themeToggleBtn) {
        themeToggleBtn.addEventListener("click", () => {
            ...
            localStorage.setItem("tiet_theme", isLight ? "light" : "dark");
    ...
    if (avatar && loginModal) {
        avatar.addEventListener("click", openLoginModal);
    }
...
// ==========================================================================
// ACADEMIC LOGIN MODAL & GOOGLE AUTH LOGIC
// ==========================================================================
window.originalModalHtml = null;
...
window.handleAcademicLogin = function() {
    const username = document.getElementById("login-username").value.trim();
    if (!username) return;
    
    // Get first character to update profile avatar
    const initial = username.substring(0, 1).toUpperCase();
    updateProfileAvatar(initial);
...
window.handleGoogleLogin = function() {
    const modalContent = document.querySelector("#login-modal .modal-card");
    if (!modalContent) return;
    
    // Save original layout to restore it when re-opening
    if (!window.originalModalHtml) {
        window.originalModalHtml = modalContent.innerHTML;
    }
    
    modalContent.innerHTML = `
        <button class="modal-close-btn" id="login-modal-close-btn" onclick="closeLoginModal()">&times;</button>
        <div class="google-chooser-container">
            <div style="display: flex; align-items: center; justify-content: center; gap: 8px; margin-bottom: 16px;">
                ...
                <span style="font-family: var(--font-heading); font-size: 15px; font-weight: 700;">Sign in with Google</span>
            </div>
            <h3 class="google-chooser-title">Choose an account</h3>
            <div class="google-account-list">
                <div class="google-account-item" onclick="selectGoogleAccount('mohitkumar5@tiet.edu', 'Mohit Kumar')">
                    <div class="google-account-avatar" style="background-color: #ef4444;">M</div>
                    <div class="google-account-details">
                        <span class="account-name">Mohit Kumar</span>
                        <span class="account-email">mohitkumar5@tiet.edu</span>
                    </div>
                </div>
                <div class="google-account-item" onclick="selectGoogleAccount('tiet.student@thapar.edu', 'Student TIET')">
                    <div class="google-account-avatar" style="background-color: #3b82f6;">S</div>
                    <div class="google-account-details">
                        <span class="account-name">Student TIET</span>
                        <span class="account-email">tiet.student@thapar.edu</span>
                    </div>
                </div>
                <div class="google-account-item" onclick="selectGoogleAccount('academic.admin@thapar.edu', 'Admin Academic')">
                    <div class="google-account-avatar" style="background-color: #10b981;">A</div>
                    <div class="google-account-details">
                        <span class="account-name">Admin Academic</span>
                        <span class="account-email">academic.admin@thapar.edu</span>
                    </div>
                </div>
            </div>
        </div>
        <p class="modal-footer-text">to continue to PolicyLens</p>
    `;
};
...
window.selectGoogleAccount = function(email, name) {
    ...
    updateProfileAvatar(initial);
    alert(`Successfully signed in with Google as: ${name} (${email})`);
    closeLoginModal();
};

function updateProfileAvatar(char) {
    const avatar = document.querySelector(".profile-avatar");
    if (avatar) {
        avatar.innerText = char;
...
function resetLoginModalContent() {
    const modalContent = document.querySelector("#login-modal .modal-card");
    if (modalContent && window.originalModalHtml) {
